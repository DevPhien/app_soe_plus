...
import 'package:soe/views/component/Html/rich_text_parser.dart';
      ...
      switch (node.localName) {
            ...
            builder: (BuildContext context) {
              if (showImages) {
                if (node.attributes['src'] != null) {
                  ...
                  precacheImage(
                    NetworkImage(node.attributes['src'].toString()),
                    context,
                    onError: onImageError,
                  );
                  return Image.network(node.attributes['src'].toString());
                } else if (node.attributes['alt'] != null) {
                  //Temp fix for https://github.com/flutter/flutter/issues/736
                  if (node.attributes['alt']!.endsWith(" ")) {
                    return Container(
                        padding: const EdgeInsets.only(right: 2.0),
                        child: Text(node.attributes['alt'].toString()));
                  } else {
                    return Text(node.attributes['alt'].toString());
                  }
                }
              }
              return Container();
        ...
        case "li":
          ...
          switch (type) {
            ...
            default: //Fallback to middle dot
              mark = SizedBox(width: 0.0, height: 0.0);
              break;
          }
          return SizedBox(
            width: width,
        ...
        case "td":
          ...
          if (node.attributes['colspan'] != null) {
            colspan = int.tryParse(node.attributes['colspan'].toString())!;
          }
          return Expanded(
            flex: colspan,
            child: Wrap(
