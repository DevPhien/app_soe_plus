...
class DuyetPhathanhVanbanController extends GetxController {
  ...
  Future<void> submit() async {
    model["type_signs"][0]["Kynhay"] = false;
    ...
    Map<String, dynamic> mdata = {
      "model": json.encode({
        ...
        "approval": approval,
        "esimsign": model["Kysim"] == true
            ? {
                "isUse": model["Kysim"],
                "reason": model["lydo"],
                "place": model["diadiem"],
                "showSigner": model["Kyboi"] ?? true,
              }
            : null
      })
    };
    ...
    try {
      var response = await http.put(
          '${Golbal.congty!.api}/api/Doc/PublishingApproval',
          data: formData);
      EasyLoading.dismiss();
  ...
  initUserVanthu() async {
    try {
      ...
      var response = await dio.post(
          "${Golbal.congty!.api}/api/Doc/GetListClericalUser",
          data: body);
      var data = response.data;
      if (data != null) {
        var tbs = List.castFrom(json.decode(data["data"]));
