...
class TaskController extends GetxController {
  ...
  initCounts() async {
    ...
    try {
      var body = {
        ...
        "duan_id": options["duan_id"],
        "s": options["s"],
        "trangthai": options["trangthai"] ?? -1,
      };
      Dio dio = Dio();
      dio.options.headers["Authorization"] = "Bearer ${Golbal.store.token}";
      dio.options.followRedirects = true;
      var response = await dio.post(
          "${Golbal.congty!.api}/api/Task/Get_CountTaskByTrangthaiNoPag",
          data: body);
  ...
  initTask(f) async {
    try {
      ...
      String url = "Task/Get_TaskByTrangthaiNoPagFilter";
      Dio dio = Dio();
      dio.options.headers["Authorization"] = "Bearer ${Golbal.store.token}";
      dio.options.followRedirects = true;
      var response =
