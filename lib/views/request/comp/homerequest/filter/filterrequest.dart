...
import 'package:soe/views/request/comp/homerequest/filter/filterrequestcontroller.dart';

class FilterRequest extends StatelessWidget {
  final FilterRequestController controller = Get.put(FilterRequestController());
  FilterRequest({Key? key}) : super(key: key);
  ...
  Widget finishdateRQ() {
    return Column(
      ...
      children: [
        ListTile(
          onTap: controller.chonFinishDate,
          leading: const Icon(FontAwesome.calendar_check_o),
          title: const Text("Ngày hoàn thành",
              style: TextStyle(fontWeight: FontWeight.bold)),
          trailing: const Icon(Feather.chevron_right),
        ),
        if (controller.startFinishDate.value != null ||
            controller.endFinishDate.value != null)
          Padding(
            padding: const EdgeInsets.only(left: 70, top: 20, bottom: 20),
            child: Wrap(
              children: [
                Obx(
                  () => controller.startFinishDate.value != null
                      ? Text(
                          DateTimeFormat.format(
                              DateTime.parse(controller.startFinishDate.value!
                                  .toIso8601String()),
                              format: 'd/m/Y'),
                          style: TextStyle(
                            color: Golbal.appColor,
                ...
                Obx(
                  () => controller.endFinishDate.value != null
                      ? Text(
                          DateTimeFormat.format(
                              DateTime.parse(controller.endFinishDate.value!
                                  .toIso8601String()),
                              format: 'd/m/Y'),
                          style: TextStyle(
                            color: Golbal.appColor,
